            ...
            <ul class="mt-6">
                ...
                <li class="relative px-6 py-3">
                    <a class="inline-flex items-center w-full text-sm font-semibold transition-colors duration-150 hover:text-gray-800 dark:hover:text-gray-200"
                       @if(isset($sidebar['custom_url']))
                       href="/admin/{{ $sidebar['url'] }}">
                        @else
                            href="{{ route('index', $sidebar['url']) }}">
                        @endif
                        <i data-feather="{{ $sidebar['icon'] }}"></i>
                        @if(isset($sidebar['custom_url']))
                            @if(request()->segment('2') ==  $sidebar['url'])
                            <span class="absolute inset-y-0 left-0 w-1 bg-purple-600 rounded-tr-lg rounded-br-lg" aria-hidden="true"></span>
                            @endif
                        @elseif(request()->segment('3') == \Illuminate\Support\Str::after(route('index', $sidebar['url']), 'admin/anka/'))
                        <span class="absolute inset-y-0 left-0 w-1 bg-purple-600 rounded-tr-lg rounded-br-lg" aria-hidden="true"></span>
                        @endif
